...
MessageBox::MessageBox(MainWindow& mw, std::string Title, std::string message, std::function<void(void*)> f) {
    ...
    title = std::make_unique<Label>(mw, Title, FontSize::Title, int(Width * 0.5), int(Height * 0.34), int(Width * 0.35), center);
    text = std::make_unique<Label>(mw, message, FontSize::Heading, int(Width * 0.5), int(Height * 0.37), int(Width * 0.35), center_top);
