...
MessageBox::MessageBox(SDL_Renderer* r, std::string title, std::string message) {
	...
	this->title = new Label(r, title, 42, int(Width * 0.5), int(Height * 0.37), center);
	text = new Label(r, message, 32, int(Width * 0.5), int(Height * 0.42), int(Width * 0.35), center);
	okButton = new Button(r, int(Width * 0.65), int(Height * 0.65), int(Width * 0.09), int(Height * 0.055), "OK", 32, bottom_right);
}
