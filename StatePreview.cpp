...
StatePreview::StatePreview(SDL_Renderer* r, int id, std::string StateName, std::string Controller, PlayerController* PC, int res[8], int pop, std::string Factories[4], std::function<void()> CloseFunc) : Screen(r) {
	int Width = GetWindowWidth(), Height = GetWindowHeight();

	LabelArr[0] = new Label(r, StateName.c_str(), 32, 0, int(Height * .55));
	...
	ImageArr[1] = new Image(r, str.c_str(), int(Width * 0.2 - 72 * Width / 1920), int(Height * .55), int(72 * Width / 1920), int(48 * Height / 1080));
