...
public:
    // Constructor
    Checkbox(MainWindow& mw, int x, int y, int Height, std::string text, int textSize, std::function<void(bool)> f = nullptr,
             int keybind = 0);
    Checkbox(MainWindow& mw, int x, int y, int Height, std::string text, int textSize, Anchor anchor,
             std::function<void(bool)> f = nullptr, int keybind = 0);
