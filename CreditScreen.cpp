...
CreditScreen::CreditScreen(MainWindow& mw, std::function<void()> fp, std::function<void(std::unique_ptr<Screen>)> fpl) :
    BackScreen(mw, fp, fpl) {
    SetupBg("Backgrounds/OldMenu.png");

    auto [Width, Height] = mw.GetWindowDimensions();
    int fontSize = int(Height / 27);

    AddDrawable<Button>(
        *main_window, int(Width * 0.85), int(Height * 0.85), int(Width * 0.08), int(Height * 0.06), "Back", int(Height / 33.75),
        [this] { Back(); }, SDLK_ESCAPE);
