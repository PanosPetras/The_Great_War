...
MainMenu::MainMenu(MainWindow& mw, std::function<void()> fp, std::function<void(std::unique_ptr<Screen>)> fpl) :
    Screen(mw, fp, fpl) {
    ...
    AddDrawable<Button>(mw, int(Width * 0.22), int(Height * 0.2), int(Width * 0.12), int(Height * 0.06), "New Game", fontSize,
                        [this] { StartGame(); });
    AddDrawable<Button>(mw, int(Width * 0.22), int(Height * 0.3), int(Width * 0.12), int(Height * 0.06), "Load Game", fontSize, fp);

    AddDrawable<Button>(mw, int(Width * 0.22), int(Height * 0.4), int(Width * 0.12), int(Height * 0.06), "Settings", fontSize,
                        [this] { ShowSettings(); });

    AddDrawable<Button>(mw, int(Width * 0.22), int(Height * 0.5), int(Width * 0.12), int(Height * 0.06), "Credits", fontSize,
                        [this] { ShowCredits(); });
